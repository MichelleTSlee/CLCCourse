...
//
        if (enemyTimeUntilChangeDir < 1) {
          ...
          dirRandomiser = myRandomNum(2) + 1; //Could be 0 otherwise. Even when I increased to 4. This random number gives random change of direction as we will see in a moment.
          enemyDirX = 0;
          enemyDirY = 0;
//
          if(enemyTimeUntilChangeDir % 2){ //Even
            if(playerX < enemyX){
              enemyDirX = -dirRandomiser; // eg -1. Makes it a negative.
              enemySpritesheetX = enemyLeftNormal;
            } else {
              enemyDirX = dirRandomiser; //eg 1. Stays positive.
              enemySpritesheetX = enemyRightNormal;
            }
          } else { //if time until enemy changes direction is an odd num
            if (playerY < enemyY){
              enemyDirY = -dirRandomiser; //eg -1. Makes it a negative.
             } else {
              enemyDirY = dirRandomiser; //eg 1. Stays positive.
             }
          }
        }

        enemyTimeUntilChangeDir--; //Has to keep counting down

        enemyX = enemyX + enemyDirX; //Wherever he started plus or minus 1- so increasing x or decreasing x
        enemyY = enemyY + enemyDirY; //Wherever he started plus or minus 1- so increasing y or decreasing y
